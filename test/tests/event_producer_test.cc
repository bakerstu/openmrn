...
TEST_F(BitEventProducerTest, SimpleOnOff) {
  storage_ = 0;
  ExpectPacket(":X195B412DN05010101FFFF0001;");
  producer_.Update(&event_write_helper1, nullptr);
  WaitForEventThread(); Mock::VerifyAndClear(&can_bus_);

  storage_ = 1;
  ExpectPacket(":X195B412DN05010101FFFF0000;");
  producer_.Update(&event_write_helper1, nullptr);
  WaitForEventThread(); Mock::VerifyAndClear(&can_bus_);

  ExpectPacket(":X195B412DN05010101FFFF0003;");
  producer2_.Update(&event_write_helper1, nullptr);
  WaitForEventThread(); Mock::VerifyAndClear(&can_bus_);

  storage_ = 3;
  ExpectPacket(":X195B412DN05010101FFFF0002;");
  producer2_.Update(&event_write_helper1, nullptr);
  WaitForEventThread(); Mock::VerifyAndClear(&can_bus_);
}
