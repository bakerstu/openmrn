...
#define SDO_ABORT_UNKNOWN_COMMAND \
  0x05040001UL  // Client/server command specifier not valid or unknown
#define SDO_ABORT_UNSUPPORTED 0x06010000UL  // Unsupported access to an object
#define SDO_ABORT_WRITEONLY 0x06010001UL    // Attempt to read a write only \
                                            // object
#define SDO_ABORT_READONLY 0x06010002UL     // Attempt to write a read only \
                                            // object
#define SDO_ABORT_NOT_EXISTS \
  0x06020000UL  // Object does not exist in the object dictionary
#define SDO_ABORT_PARAINCOMP \
  0x06040043UL  // General parameter incompatibility reason
#define SDO_ABORT_ACCINCOMP \
  0x06040047UL  // General internal incompatibility in the device
#define SDO_ABORT_TYPEMISMATCH                                                \
  0x06070010UL  // Data type does not match, length of service parameter does \
                // not match
#define SDO_ABORT_UNKNOWNSUB 0x06090011UL  // Sub-index does not exist
#define SDO_ABORT_VALUE_RANGE \
  0x06090030UL  // Value range of parameter exceeded (only for write access)
#define SDO_ABORT_TRANSFER \
  0x08000020UL  // Data cannot be transferred or stored to the application
#define SDO_ABORT_LOCAL                                                    \
  0x08000021UL  // Data cannot be transferred or stored to the application \
                // because of local control
#define SDO_ABORT_DEVSTAT                                                  \
  0x08000022UL  // Data cannot be transferred or stored to the application \
...
#define CAN_SDOREQ_HANDLED_SEND \
  1  // processed in callback, auto-send returned msg
#define CAN_SDOREQ_HANDLED_NOSEND \
  2  // processed in callback, don't send response
...
typedef struct _CAN_CALLBACKS {
  ...
  uint32_t (*CANOPEN_sdo_seg_read)(uint16_t index, uint8_t subindex,
                                   uint8_t openclose, uint8_t *length,
